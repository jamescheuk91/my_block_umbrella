defmodule Utilities.Map do
  ...
  # atoms
  def to_atom_keys(struct = %{__struct__: _}) do
    struct
  end

  def to_atom_keys(map = %{}) do
    map
    |> Enum.map(fn {k, v} -> {String.to_atom(k), to_atom_keys(v)} end)
    |> Enum.into(%{})
  end

  # Walk the list and atomize the keys of
  # of any map members
  def to_atom_keys([head | rest]) do
    [to_atom_keys(head) | to_atom_keys(rest)]
  end

  def to_atom_keys(not_a_map) do
    not_a_map
  end
